...
public struct ___VARIABLE_modelName___ListScreen: ViewProtocol {
    public var body: some View {
        ...
        .task(priority: .background) { await viewModel.onAppear() }
        .refreshable { await viewModel.onRefresh() }
        .navigationMove($viewState.destination)
        .onChange(of: viewState.searchTerm) {
            Task { await viewModel.onChangeSearchTerm(oldValue: $0, newValue: $1) }
        }
    }
            ...
            LazyVStack(spacing: .zero) {
                ForEach(___VARIABLE_modelPluralVariableName___) { ___VARIABLE_modelVariableName___ in
                    ___VARIABLE_modelName___Row(___VARIABLE_modelVariableName___, isCompact: viewState.storage.isCompactRow) {
                        Task { await viewModel.onTapDetail___VARIABLE_modelName___(___VARIABLE_modelVariableName___) }
                    }
    ...
    private var emptyContent: some View {
        if viewState.searchTerm.isEmpty {
            ___VARIABLE_modelName___EmptyView(action: { Task { await viewModel.onCreate() } })
        } else {
            ContentView(
    ...
    private func toolbarContent() -> some ToolbarContent {
        ToolbarItemGroup(placement: .primaryAction) {
            ...
            Menu {
                ...
                Button {
                    Task { await viewModel.onTapSearch() }
                } label: {
                    Label {
    ...
    private func contextMenu(___VARIABLE_modelVariableName___: ___VARIABLE_modelName___) -> some View {
        Button(role: .destructive, action: { Task { await viewModel.delete___VARIABLE_modelName___(___VARIABLE_modelVariableName___) } }) {
            Label {
